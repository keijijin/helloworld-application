...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "helloworld-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name helloworld-application-service apps/helloworld-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app helloworld-application-service:1.0-SNAPSHOT
		call oc expose svc/helloworld-application-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd helloworld-application-service
		cd target
