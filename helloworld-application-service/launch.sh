...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "helloworld-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name helloworld-application-service apps/helloworld-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app helloworld-application-service:1.0-SNAPSHOT
    oc expose svc/helloworld-application-service
else

	echo "Launching the application locally..."
	pattern="helloworld-application-service"
	files=( $pattern )
	cd ${files[0]}
